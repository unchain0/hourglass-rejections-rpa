...
func TestPriority_FinishAuthentication_UsesExistingCookiesAndHeader(t *testing.T) {
	a := &Authenticator{
		baseURL:   "https://example.com",
		hgLogin:   "existing-hg",
		xsrfToken: "existing-xsrf",
		httpClient: &mockHTTPClient{do: func(req *http.Request) (*http.Response, error) {
			cookie, err := req.Cookie("hglogin")
			require.NoError(t, err)
			assert.Equal(t, "existing-hg", cookie.Value)
			assert.Equal(t, "existing-xsrf", req.Header.Get("X-Hourglass-XSRF-Token"))
			header := make(http.Header)
			header.Add("Set-Cookie", (&http.Cookie{Name: "hglogin", Value: "new-hg", Expires: time.Now().Add(2 * time.Hour)}).String())
			header.Add("Set-Cookie", (&http.Cookie{Name: "X-Hourglass-XSRF-Token", Value: "new-xsrf", Expires: time.Now().Add(time.Hour)}).String())
			return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader("ok")), Header: header}, nil
		}},
	}

	tokens, err := a.finishAuthentication(&AssertionResponse{Type: "public-key"})
	require.NoError(t, err)
	assert.Equal(t, "new-hg", tokens.HGLogin)
	assert.Equal(t, "new-xsrf", tokens.XSRFToken)
}
