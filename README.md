...
The default compose file starts the main `rpa` service with a persisted authentication volume. Leave `CHROME_PROFILE_DIR` unset unless you also imported a previously authenticated browser profile. A headless server cannot complete the first interactive Google/WebAuthn login; it renews authentication from the files generated by `setup-auth`.
